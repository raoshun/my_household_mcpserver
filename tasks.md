...
- **対象要件**: FR-001〜FR-022, NFR-001〜NFR-024
...
- [ ] **TASK-1103**: 資産クラス関連エンドポイント
  - [ ] `GET /api/assets/classes` - 資産クラス定義一覧取得
  - [ ] テスト実装
  - **見積**: 0.5d

- [ ] **TASK-1104**: 資産レコード CRUD エンドポイント
  - [ ] `GET /api/assets/records` - フィルタリング機能付き一覧取得
  - [ ] `POST /api/assets/records` - 新規登録
  - [ ] `PUT /api/assets/records/{record_id}` - 編集
  - [ ] `DELETE /api/assets/records/{record_id}` - 削除（論理削除）
  - [ ] 入力値検証実装
  - [ ] テスト実装
  - **見積**: 1.0d

- [ ] **TASK-1105**: 資産集計エンドポイント
  - [ ] `GET /api/assets/summary` - 月末時点の資産クラス別残高集計
  - [ ] `GET /api/assets/allocation` - 資産配分（月末時点）
  - [ ] テスト実装
  - **見積**: 0.75d

- [ ] **TASK-1106**: エクスポート エンドポイント
  - [ ] `GET /api/assets/export?format=csv` - CSV エクスポート
  - [ ] テスト実装
  - **見積**: 0.5d

- [ ] **TASK-1107**: HTTPサーバー統合
  - [ ] `src/household_mcp/web/http_server.py` に資産ルート追加
  - [ ] CORS設定確認
  - [ ] エラーハンドリング統一
  - [ ] 統合テスト
  - **見積**: 0.5d
...
- [ ] **TASK-1108**: API 統合テスト
  - [ ] `tests/integration/test_assets_api.py` 実装
  - [ ] 全エンドポイントの正常系・異常系テスト
  - [ ] エッジケース（存在しないレコード、無効な入力値等）
  - **見積**: 0.75d

### 11.3 フロントエンド実装 (TASK-1109～1113)

- [ ] **TASK-1109**: assets.html ページ作成
  - [ ] HTML 構造実装（フォーム、テーブル、グラフ、サマリー各セクション）
  - [ ] CSS スタイリング（`frontend/css/assets.css` 作成）
  - [ ] レスポンシブデザイン対応
  - **見積**: 1.0d

- [ ] **TASK-1110**: JavaScript ロジック実装（js/assets.js）
  - [ ] `AssetManager` クラス実装
  - [ ] CRUD UI イベントハンドラ
  - [ ] グラフ描画（Chart.js使用）
  - [ ] 期間指定・プリセット機能
...
- [ ] **TASK-1111**: 統計サマリーセクション実装
  - [ ] 合計資産額、前月比、最大資産額の計算・表示
  - [ ] リアルタイム更新対応
  - **見積**: 0.5d
...
- [ ] **TASK-1114**: ユニットテスト
  - [ ] `tests/unit/assets/test_manager.py` - CRUD操作の単体テスト
  - [ ] `tests/unit/assets/test_analyzer.py` - 集計ロジックのテスト
  - [ ] カバレッジ: 85% 以上
  - **見積**: 0.75d

- [ ] **TASK-1115**: 手動UI/UX テスト
  - [ ] 資産登録フロー確認
  - [ ] グラフ描画確認（複数月表示、期間指定等）
...
- [ ] **TASK-1116**: パフォーマンス・メモリ検証
  - [ ] NFR-022: レスポンス 1秒以内確認
  - [ ] NFR-023: グラフ生成 3秒以内確認
  - [ ] NFR-024: 1000件で月次集計 1秒以内確認
  - [ ] メモリプロファイリング（大規模データセット）
  - **見積**: 0.75d

### 11.5 ドキュメント・統合 (TASK-1117～1119)

- [ ] **TASK-1117**: API ドキュメント作成
  - [ ] `docs/api.md` に資産管理 API セクション追加
  - [ ] エンドポイント仕様、リクエスト/レスポンス例
  - [ ] エラーレスポンス例
  - [ ] 使用例（curl等）
  - **見積**: 0.5d

- [ ] **TASK-1118**: ユーザードキュメント作成
  - [ ] `docs/usage.md` に資産管理セクション追加
  - [ ] スクリーンショット・GIF（オプション）
  - [ ] 操作手順説明
  - **見積**: 0.5d

- [ ] **TASK-1119**: README 更新
  - [ ] `backend/README.md` 資産管理API説明追加
  - [ ] `frontend/README.md` 資産管理UIについて記載
  - [ ] `README.md`（ルート）に機能概要追加
  - **見積**: 0.25d
