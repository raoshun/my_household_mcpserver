...
3. ⏳ **優先2: data_tools.py 分割**
   - ⏳ transaction_tools.py, asset_tools.py 新規作成
   - ⏳ 各ツールクラスを新ファイルに移動
   - ⏳ data_tools.py を統合・再エクスポートファイルに改造
   - ⏳ テストインポート更新
...
| 対象                   | 現状           | 目標                         | 効果     |
| ---------------------- | -------------- | ---------------------------- | -------- |
| エラー処理             | 分散           | `errors/handlers.py`         | 保守性 ↑ |
| 日付・金額フォーマット | `utils/`       | 確定版へ統一                 | DRY準拠  |
| DB クエリ生成          | 複数箇所       | `database/query_builders.py` | 性能 ↑   |
| Pydantic スキーマ      | モジュール分散 | `schemas/` ディレクトリ化    | 型安全 ↑ |
...
| **合計**  | **8.0d** | **-**  | **37% 進捗** | **NFR-016,017** |

**進捗**: 4.0d / 8.0d = **50% 完了** (TASK-1601 + 1602 + 1603)
