...
- [ ] `src/household_mcp/analysis/fire_calculator.py` 新規作成
  - [ ] `calculate_fire_index()`: FIRE到達月計算（複利・インフレ）
...
- [ ] 月利が年利から正しく導出される
- [ ] 複利効果がシミュレーションに反映される
- [ ] インフレ調整が正確（2% インフレで実質資産が 98% になる）
- [ ] シナリオ別に異なる到達月が出力される
- [ ] 計算時間 < 100ms

**成果物**: `fire_calculator.py` (200 行)、テスト 5 個
...
- [ ] `src/household_mcp/analysis/scenario_simulator.py` 新規作成
  - [ ] `ScenarioSimulator` クラス実装
  - [ ] 支出削減シナリオ
  - [ ] 収入増加シナリオ
  - [ ] 複合シナリオ
  - [ ] 推奨シナリオ選定（ROI = 効果 / 難易度）
...
- [ ] 各シナリオで異なる到達月が出力される
- [ ] 推奨シナリオが適切に選定される
- [ ] パフォーマンス < 500ms（5 シナリオ）

**成果物**: `scenario_simulator.py` (250 行)、テスト 4 個
...
- [ ] `src/household_mcp/analysis/expense_pattern_analyzer.py` 新規作成
  - [ ] 定期支出分類（3ヶ月以上、変動率 < 5%）
  - [ ] 変動支出分類（平均 ± 2σ 範囲内）
  - [ ] 異常支出検出（平均 + 2σ を超える）
  - [ ] 季節性指数計算（月別指数）
...
- [ ] 季節性指数が正確に計算される
- [ ] トレンド傾きが正確（R² で適合度検証）
- [ ] パフォーマンス < 300ms（12ヶ月データ）
- [ ] 3ヶ月未満のデータは適切に処理される

**成果物**: `expense_pattern_analyzer.py` (300 行)、テスト 6 個
...
- [ ] MCP リソース追加（3 個）
  - [ ] `data://financial_independence`: FIRE計算結果
  - [ ] `data://scenarios`: シナリオ分析結果
  - [ ] `data://expense_patterns`: 支出パターン分析結果

- [ ] MCP ツール追加（3 個）
  - [ ] `calculate_fire_index()`: FIRE計算
  - [ ] `simulate_scenarios()`: シナリオ分析
  - [ ] `analyze_spending_patterns()`: パターン分析
...
**成果物**: `server.py` 拡張（150 行）、API テスト
...
**成果物**: `test_phase15_integration.py` (400 行)、テスト 15+ 個
