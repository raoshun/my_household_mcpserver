"""Integration tests for CSV-based annual expense calculation in FireSnapshotService.

These tests are migration of the previous root-level tests to the integration
folder. They rely on the real `HouseholdDataLoader` contract and are marked as
integration so CI can run unit tests separately from heavier CSV-backed
integration checks.
"""
...
if TYPE_CHECKING:
    # No runtime-only imports required; keep this block for future type hints
    pass
...
class TestCalculateAnnualExpenseFromCSV:
    ...
    def test_prefers_12_month_over_6_month(
        ...
        """Test prefers 12-month calculation when both
        6 and 12 months available.
        """
...
class TestRecalculateFICacheIntegration:
    ...
    def test_passes_snapshot_date_to_expense_estimation(
        ...
        q = mock_query.order_by.return_value.filter.return_value
