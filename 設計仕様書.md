...
FIRE進捗ダッシュボードに表示する資産スナップショットを、ユーザーが任意のタイミングでMCP経由で登録できるようにする。登録フォーマットは添付画像にある表に準じ、指定されている資産カテゴリのみを対象とする。

### 12.2 登録対象カテゴリ

- `total`（合計資産額）
- `cash_and_deposits`（預金・現金・貯蓄）
- `stocks_cash`（株式（現物））
- `stocks_margin`（株式（信用））
...
上記以外のカテゴリを含むスナップショットは受け付けず、必ずこの一覧に限定された形で登録を行う。

### 12.3 データ登録/参照フロー

1. MCPツールまたはRESTエンドポイント `register_fire_snapshot` を通じて、`snapshot_date`（YYYY-MM-DD）と上記カテゴリ値を含むJSONを送信する。ユーザーが任意のタイミングで送信することで登録が完了し、定期バッチ等は必要としない。
2. 受信したデータは `fi_progress_cache` テーブルの `snapshot_date` をキーに一意に保存する。既存日付と重複する場合は上書きし、数値以外や負の値はバリデートエラーとする。
3. 登録済データは `financial_independence` API および MCP 工具がそのまま参照する。カテゴリごとにマッピングされたカラムと一致させることで、FIRE分析やダッシュボードのスナップショット表示を更新する。

### 12.4 年間支出算出（FR-023-1A）🆕

**目的**: FIRE進捗計算で家計簿CSVの実支出を使用し、正確なFIRE目標資産額を算出

**算出方法**:

1. **優先**: 家計簿CSVから直近12ヶ月の実支出を集計
   - スナップショット基準日から遡って12ヶ月分のCSVデータを読み込み
   - `計算対象=1` かつ `金額（円）<0` のレコードを抽出
   - 月別に支出を集計し、12ヶ月分の合計を算出
2. **代替**: データ不足時の処理
   - 6ヶ月以上のデータがある場合: 月平均 × 12で年換算
   - 6ヶ月未満の場合: 資産額の4%で推定（フォールバック）
3. **手動**: ユーザー指定の年間支出（最優先）

**実装**:

- `FireSnapshotService._calculate_annual_expense_from_csv()` メソッド
- `HouseholdDataLoader.load_many()` を使用してCSVデータを取得
- エラー時は `_estimate_annual_expense()` にフォールバック
- スナップショット登録時に `fi_progress_cache.annual_expense` に保存

**FIRE目標資産額の計算**:

```
年間支出 = CSV集計値（優先） or 資産額 × 4%（代替） or ユーザー指定値（最優先）
FIRE目標資産額 = 年間支出 × 25
```

**MCPツール**:

- `get_annual_expense_breakdown`: 年間支出の月別・カテゴリ別内訳を取得
- `compare_actual_vs_fire_target`: 実支出とFIRE目標の比較分析
- `get_financial_independence_status`: CSVベースの年間支出を自動使用（拡張済み）

### 12.5 補完戦略

参照時に指定日と一致するスナップショットが存在しない場合、直近の前後登録値を用いてカテゴリ単位で線形補完を行い、暫定的な値を提供する。補完処理は抽象化レイヤー（補完プロバイダーインターフェース）で実装し、将来的に移動平均や回帰分析等へ切り替える際はプロバイダー実装の差し替えだけで済むようにする。補完に必要な過去/未来の値が不足するカテゴリには、最後に登録された値をそのまま採用する。

### 12.6 線形補完の可換性

現在の仕様では線形補完を暫定的手法として採用するが、補完プロバイダーを設定やDIで差し替え可能に設計し、今後のアルゴリズム変更に柔軟に対応できるようにしておく。
