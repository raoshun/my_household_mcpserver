...
from household_mcp.visualization.chart_generator import ChartGenerator  # noqa: E402
...
def benchmark_image_generation():
    ...
    tests = [
        (
            "pie",
            lambda: generator.create_monthly_pie_chart(monthly_data, "テスト円グラフ"),
        ),
        (
            "line",
            lambda: generator.create_category_trend_line(
                trend_data, "食費", "テスト折れ線グラフ"
            ),
        ),
        (
            "bar",
            lambda: generator.create_comparison_bar_chart(
                comparison_data, "テスト棒グラフ"
            ),
        ),
    ]
