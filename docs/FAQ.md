...
### Q49: 重複検出はどうやって実行しますか?

**A**: AIエージェント経由またはWebアプリから実行できます。
...
**Webアプリ経由:** メインページ上部の「重複検出」タブをクリック → 自動検出ボタンを押して候補を表示 → 左パネルから候補を確認・判定

### Q50: 重複判定の精度はどうですか?

**A**: デフォルト設定では日付の差が±0日、金額の差が±0円で、類似度スコアが80%以上の取引を候補として抽出します。許容範囲を広げたい場合は `date_tolerance_days` や `amount_tolerance_pct` を指定してください。

### Q51: 誤検出で大事な取引を削除してしまいました

**A**: 削除前に必ず確認画面が表示されます。万が一削除してしまった場合は、`backend/data/household.db` のバックアップから復元するか、CSVファイルを再インポートしてください。

### Q52: 重複検出の対象外にしたい取引があります

**A**: Webアプリの判定画面で「スキップ」ボタンを押すと、その候補を一時的にスキップできます。

### Q53: Webアプリで重複検出ページが表示されません

**A**: 以下を確認してください：

1. バックエンドが起動しているか：`curl http://localhost:8000/health`
2. `/api/duplicates/` エンドポイントが利用可能か：`curl http://localhost:8000/api/duplicates/stats`
3. ブラウザキャッシュをクリア

### Q54: 大量の重複候補が検出されました

**A**: 許容範囲を厳しくして再実行してください。`min_similarity_score` を0.95以上に設定すると、信頼度の高い候補のみに絞れます。

### Q55: 重複検出の統計情報を見たいです

**A**: Webアプリの「重複検出」ページの「統計」タブをクリックすると、検出件数・確認件数・削除件数が表示されます。APIから直接呼び出す場合は `curl http://localhost:8000/api/duplicates/stats` を実行してください。
