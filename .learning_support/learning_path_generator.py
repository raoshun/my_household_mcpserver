...
from understanding_checker import (
    LearningDataManager,
    TDDProficiency,
    UnderstandingLevel,
)
...
class LearningPathGenerator:
    ...
    def _analyze_progress_and_adjust(
        self, target_concept: str, steps: List[LearningStep]
    ) -> List[LearningStep]:
    ...
    def _adjust_review_schedule(self, concept: str):
        ...
        schedule[concept] = [today] + [
            str(self._add_days(today, d)) for d in intervals[1:]
        ]
    ...
    def generate_learning_path(
        self, target_concept: str, learning_style: str = "balanced"
    ) -> LearningPath:
    ...
    def _analyze_knowledge_gap(
        self, target_concept: str, current_record: Optional[Any]
    ) -> Dict[str, Any]:
        """知識ギャップを分析"""
        ...
        for prereq in target_info["prerequisites"]:
            ...
            if not prereq_record or prereq_record.understanding_level in [
        ...
        if missing_prerequisites:
            ...
            identified_gap = (
                f"{target_concept}の前提知識: {', '.join(missing_prerequisites)}"
            )
        else:
            # 直接実装可能
    ...
    def _generate_learning_steps(
        self, gap_analysis: Dict[str, Any], learning_style: str
    ) -> List[LearningStep]:
        """学習ステップを生成"""
        steps = []
        step_counter = 1
        ...
        target_step = self._create_learning_step(
            gap_analysis["starting_point"], step_counter, learning_style
        )
    ...
    def _create_learning_step(
        self, concept: str, step_number: int, learning_style: str
    ) -> LearningStep:
        """個別の学習ステップを作成"""
        ...
        # 学習方法を選択
        learning_method = concept_info["learning_methods"].get(
            learning_style, "バランス型学習"
        )
    ...
    def _save_learning_path(self, learning_path: LearningPath):
        ...
        self.data_manager.data["learning_paths"][learning_path.target_concept] = asdict(
            learning_path
        )
        self.data_manager._save_data()
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python learning_path_generator.py <target_concept> [learning_style] [--check-review]"
        )
