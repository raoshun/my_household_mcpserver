...
def load_csv_from_month(
    ...
    if year is None: # yearが指定されていない場合は全てのデータを対象とする
        df = pd.concat([
            ...
            for f in os.listdir(src_dir) if f.endswith(".csv")
        ])
        return df
    elif month is None: # yearは指定されているがmonthが指定されていない場合はその年の全てのデータを対象とする
        files = [f for f in os.listdir(src_dir) if f.startswith(f"収入・支出詳細_{year}-")]
        df = pd.concat([
            pd.read_csv(os.path.join(src_dir, f), **common_kwargs)
    ...
    csv_filename = f"収入・支出詳細_{year}-{month:02d}-01_{year}-{month:02d}-{end_day}.csv"
