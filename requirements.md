...
- バージョン: v1.6（フェーズ 16: 収入分析・強化FIRE計算）
- 日付: 2025-11-16
- 作成者: GitHub Copilot
...
### FR-031: FIRE進捗スナップショット登録

**目的**: SDD §12 に示す資産スナップショット表の形式を準拠したデータを、ユーザー／エージェントが任意に MCP および REST 経由で登録し、Fire 進捗ダッシュボードと API が参照可能な状態に保つ。

**詳細**:

- 入力: `snapshot_date` (YYYY-MM-DD) と以下のカテゴリ値（単位: 円）を含む JSON。
  - `cash_and_deposits`, `stocks_cash`, `stocks_margin`, `investment_trusts`, `pension`, `points`
- リクエストに `total` を含めてはいけない。`total` はサーバー側で一時的にカテゴリ合計から計算し、`fi_progress_cache` やダッシュボード応答で使用する。
- それ以外のカテゴリやフォーマットは拒絶し、数値・非負・通貨単位チェックを行う。カテゴリ合計はサーバー側で算出され、`total` 欄は存在しない前提で処理する。
- 登録リクエストは認証済みユーザー/エージェントのみが `POST /api/financial-independence/snapshot` または `register_fire_snapshot` (MCP Tool) で送信。`snapshot_date` をキーに `fire_asset_snapshots` テーブルへ挿入または更新し、同日データが既存の場合は上書き。
- 保存後、当該日付を起点に `FinancialIndependenceAnalyzer` がカテゴリ合計を使って `total` を算出し、`fi_progress_cache` を再計算して進捗率・月間成長率・到達月数などをキャッシュ。これらの値が `financial_independence` API および MCP ツールで即座に参照可能となる。
- ダッシュボードや API が `snapshot_date` を指定したときにスナップショットが存在しない場合、直近登録値の前後をカテゴリ単位で線形補完し、暫定値を生成。補完には `SnapshotInterpolator` インターフェースを用い、将来のアルゴリズム（移動平均、回帰）への差し替えを容易にする。
- 補完に必要な前後値が不足するカテゴリは、直近登録値をそのまま再利用し、欠損データの影響を最小化。

**受け入れ条件**:

- 入力に許容されるカテゴリ以外、非数値、負値、`total` キーが含まれる場合には 400 エラーを返し、データベースは更新しない。
- 認証済の登録リクエストは `fire_asset_snapshots` に保存され、同日データに対して再登録時は上書きされる。
- ダッシュボード/API が未登録の日付を要求した際、線形補完されたカテゴリ値と計算済み進捗指標が返される。
- 補完ロジックは `SnapshotInterpolator` を差し替えるだけで他実装へ置き換え可能である。（Linear = デフォルト）
- 登録と補完により `fi_progress_cache` が更新され、進捗率・成長率・到達月数などが新しいスナップショットの値を反映した状態でキャッシュされる。

**テスト項目**:

- TS-041: `register_fire_snapshot` API/MCP 呼び出しでサンプルデータを登録し、`fire_asset_snapshots` に保存されることを確認。
- TS-042: 未登録日を要求しても線形補完されたカテゴリ値と進捗指標が API/ダッシュボードに返ることを E2E で確認。
- TS-043: 不正カテゴリ・負値・合計不一致の入力は 400 で拒否され、DB に影響しない。
- TS-044: `SnapshotInterpolator` を差し替えたモック実装で補完結果を変更できることを単体テストで確認。
- TS-045: `total` フィールドを含む登録リクエストは 400 を返し、`fire_asset_snapshots` は更新されないことを API/MCP テストで確認。
受け入れ条件:
...
NFR-016 リポジトリ肥大化対策: AI Codingの性能低下を抑制するため、定期的なコード最適化と整理を実施。単一ファイルの行数が500行を超えた場合は機能別・責任別に分割、共通性の高いコンポーネント・関数・ユーティリティを積極的に共通化、不要な依存関係・デッドコードを削除、テストファイルの適切な分割・整理（1ファイル200行以上）、定期的なリファクタリング（Phase終了時または月次）で品質保持、REFACTORING.md に実施記録を残す
NFR-017 エディターレベルの品質確保: 開発環境でのコード品質を維持し、エディター表示の問題・警告を積極的に解消。Pylance/Ruff等の静的解析ツール警告を最小化（新規追加時は0警告）、全関数・メソッドに型アノテーション付与、未使用インポート・変数を即座削除、複雑な関数・クラスに Docstring 記載、リリース前に Problems パネルの High/Medium エラーを0に、Linter・Formatter・TypeChecker を PR 前に実行、CI/CD でコード品質ゲートを実装
...
TS-021 ファイル行数チェック（NFR-016）: 500行以上のファイルがない（新規ファイルは1ファイル500行以下）
TS-022 コード重複チェック（NFR-016）: 同等の関数・ユーティリティの重複が最小（70%以上共通化）
TS-023 エディター問題チェック（NFR-017）: Pylance/Ruff における High/Medium 警告が0（新規コンポーネント）
TS-024 型チェック完全性（NFR-017）: 全ての関数・メソッドに型アノテーション完備、union型を含む
TS-025 Docstring 整備（NFR-017）: 複雑な関数・クラス（10行以上の関数、複数メソッドを持つクラス）に description・params・returns を記載
TS-026 未使用インポート削除（NFR-017）: ruff check 実行で未使用インポートがない状態を維持
TS-027 REFACTORING.md 記録（NFR-016）: リファクタリング実施時に対象ファイル・変更理由・完了日を記録
TS-028 FIRE目標資産額計算: 直近12ヶ月平均月支出 × 12 × 25 が正しく計算される
TS-029 月別資産増加率: 複数月の資産推移から月利が正しく算出される
TS-030 移動平均トレンド: 3ヶ月移動平均により季節性ノイズが低減される
TS-031 到達月数予測: 現在純資産、月利、目標資産額から到達月数が数学的に正確に計算される
TS-032 シナリオ別予測: 悲観/中立/楽観シナリオの到達予定日が適切に異なる（悲観 > 中立 > 楽観）
...
TS-041 FIREスナップショット登録: `register_fire_snapshot` API/MCP が許容カテゴリ値を受け取り `fire_asset_snapshots` に保存し、`snapshot_date` で上書き可能
TS-042 FIREスナップショット補完: 未登録日でも線形補完によるカテゴリ値とキャッシュ済進捗指標が `financial_independence` API/ダッシュボードに返る
TS-043 FIREスナップショット入力検証: 不許可カテゴリ・負値・合計不一致を含むリクエストは 400 で拒否され、DB は更新されない
TS-044 補完プロバイダー入れ替え: `SnapshotInterpolator` を差し替えたモックで補完結果が変更されることを単体テストで確認
...
- 年支出額は家計簿データから以下の優先順位で算出：
  1. **優先**: 家計簿CSVから直近12ヶ月の実支出を集計（FR-023-1A）
  2. **代替**: データ不足時（6ヶ月未満）は資産額の4%で推定
  3. **手動**: ユーザーが独自の年支出額を設定可能（最優先）
- 支出額は、計算対象フラグ（`計算対象=1`）かつマイナス金額（`金額<0`）のレコードのみを対象とする

**FR-023-1A: 家計簿CSVからの年間支出算出** 🆕

- **目的**: 実際の家計簿データから正確な年間支出を算出し、FIRE進捗を正しく判定
- **算出方法**:
  1. スナップショット基準日から遡って12ヶ月分のCSVデータを読み込み
  2. `計算対象=1` かつ `金額（円）<0` のレコードを抽出（HouseholdDataLoaderの_post_processで自動実施）
  3. 月別に支出を集計し、12ヶ月分の合計を算出
  4. データ不足時の代替ロジック:
     - 6ヶ月以上のデータがある場合: 月平均 × 12で年換算
     - 6ヶ月未満の場合: DataSourceErrorを発生させ、資産額ベースの推定にフォールバック
- **連携**:
  - `FireSnapshotService._calculate_annual_expense_from_csv()` で実装
  - `HouseholdDataLoader.load_many()` を使用してCSVデータを取得
  - エラー時は従来の `_estimate_annual_expense()` にフォールバック
- **キャッシュ**: スナップショット登録時に `fi_progress_cache.annual_expense` に保存
...
- 🆕 家計簿CSVから実支出が算出される（12ヶ月分データがある場合）
- 🆕 データ不足時（6ヶ月未満）は適切にエラーハンドリングされる
- 🆕 資産額ベース推定へのフォールバックが機能する
- 🆕 MCPツール `get_annual_expense_breakdown` で月別・カテゴリ別内訳を取得できる
...
- **承認日**: 2025-11-05
- **実装完了日**: 2025-11-08
- **概要**: 現在資産と月貯蓄から、指定された目標資産に到達するまでの月数を計算。複利効果とインフレを考慮した3シナリオ（悲観/中立/楽観）での予測を実施
- **成果物**: `fire_calculator.py` (245行)、テスト 20個、カバレッジ 88%
- **パフォーマンス**: < 100ms ✅
- **次ステップ**: TASK-1505 統合テスト
...
- **承認日**: 2025-11-05
- **実装完了日**: 2025-11-08
- **概要**: 支出削減・収入増加など複数のシナリオを比較し、ROI（効果/難易度）に基づいて最適な改善施策を推奨。5つのデフォルトシナリオ（10%/20%削減、収入増加、複合）をサポート
- **成果物**: `scenario_simulator.py` (256行)、テスト 12個、カバレッジ 100%
- **パフォーマンス**: < 500ms（5シナリオ） ✅
- **次ステップ**: TASK-1505 統合テスト
...
- **承認日**: 2025-11-05
- **実装完了日**: 2025-11-08
- **概要**: 支出を定期/変動/異常の3つに自動分類。12ヶ月以上のデータから季節性指数を計算、トレンド分析で増減傾向を把握。異常支出検出（σ閾値）により不正検知に活用可能
- **成果物**: `expense_pattern_analyzer.py` (299行)、テスト 14個、カバレッジ 98%
- **パフォーマンス**: < 300ms（12ヶ月データ） ✅
- **次ステップ**: TASK-1505 統合テスト

### FR-028: Phase 15 分析ツール MCP 統合

- **ステータス**: ✅ 承認済み・実装完了
- **承認日**: 2025-11-05
- **実装完了日**: 2025-11-08
- **概要**: FR-025〜FR-027 の3つのツールを MCP リソースとして登録。get_tools() で公開、会話インターフェース経由で利用可能
- **成果物**: server.py 拡張 (~270行)、統合テスト 6個
- **テスト結果**: 6/6 PASSED ✅
- **次ステップ**: TASK-1505 デリバリー準備
...
- FR-001〜FR-028: ✅ 承認済み・実装完了
- FR-024（DB統合）: 🔷 検討中（Phase 16 予定）
- FR-031: 🔷 検討中（Phase 15 FIRE進捗スナップショット登録）
- NFR-001〜NFR-032: ✅ 基本要件承認済み
...
- **NFR-037**: 収入分析の計算は月次/年次で 1 秒以内に完了
- **NFR-038**: 貯蓄率計算の数値精度は小数第 2 位まで（パーセント）
- **NFR-039**: FIREシミュレーションは最大5シナリオを 3 秒以内に計算
- **NFR-040**: 収入データのキャッシングは 1 時間有効（CSV再読み込み時は無効化）
- **NFR-041**: 不動産キャッシュフロー計算は物件数10件まで対応
- **NFR-042**: シナリオ保存ファイルは最大 100KB まで（JSONフォーマット）

---

---

### FR-032: 収入分析とキャッシュフロー管理

**目的**: 家計簿CSVデータから収入を分析し、支出との対比でキャッシュフローを管理する機能を提供。FIRE計算の精度向上と、実際の資産形成ペースの可視化を実現する。

**詳細**:

#### FR-032-1: 収入カテゴリ分類

- CSV データから `金額（円）> 0` かつ `計算対象 = 1` のレコードを収入として抽出
- 収入を以下の 5 カテゴリに自動分類:
  1. **給与所得**: 大項目「給与」または中項目に「給与」「賞与」を含む
  2. **事業所得**: 大項目「事業収入」または中項目に「事業」を含む
  3. **不動産所得**: 大項目「不動産」または中項目に「家賃収入」「不動産収入」を含む
  4. **配当・利子所得**: 大項目「金融収入」または中項目に「配当」「利子」「分配金」を含む
  5. **その他収入**: 上記以外のすべての収入（臨時収入、還付金など）
- 分類ルールは `income_categories.json` で管理し、ユーザーがカスタマイズ可能

**受け入れ条件**:

- 2022-01 から 2025-06 までの 42 ヶ月分の CSV データから収入を正しく抽出できる
- 5 カテゴリへの自動分類が 95% 以上の精度で動作する
- カスタム分類ルールを追加・編集できる

#### FR-032-2: 月次・年次収入サマリー

- 指定期間（月次/年次）の収入を集計し、以下の情報を提供:
  - 総収入額
  - カテゴリ別収入額と構成比率
  - 前月比/前年比の変化率
  - 平均月収（年次サマリーの場合）
- グラフ出力: 円グラフ（カテゴリ構成比）、折れ線グラフ（推移）

**受け入れ条件**:

- `get_monthly_income_summary(2024, 7)` で 2024年7月の収入サマリーを取得できる
- 前月比・前年比が正しく計算される
- グラフが PNG/JSON 形式で出力される

#### FR-032-3: 貯蓄率計算

- 月次/年次の「貯蓄率」を計算: `貯蓄率 = (収入 - 支出) / 収入 × 100`
- 以下の指標も提供:
  - 可処分所得: 収入から固定費を除いた額
  - 変動費率: 変動費 / 可処分所得
  - 貯蓄額: 収入 - 支出
...
- CSV データから正しく貯蓄率を計算できる（実測値 ¥20,192,901 支出を考慮）
- 固定費・変動費の分類が正しく動作する
- 貯蓄率が負の場合（赤字）も正しく表示される

#### FR-032-4: 不動産キャッシュフロー分析

- 不動産所得と不動産関連支出を照合し、ネットキャッシュフローを計算
- 以下の情報を提供:
  - 不動産収入（家賃収入など）
  - 不動産支出（管理費、修繕費、ローン返済など）
  - ネットキャッシュフロー: 収入 - 支出
  - ROI（投資利回り）: ネットキャッシュフロー / 初期投資額
- 複数物件の管理（物件 ID による分類）

**受け入れ条件**:

- 不動産収入と支出が正しく照合される
- ネットキャッシュフローとROIが計算される
- 複数物件のデータを個別に管理・分析できる

---

### FR-033: 強化されたFIREシミュレーション

**目的**: 収入データを活用し、より現実的なFIRE達成シナリオをシミュレーションする。支出のみに基づく従来の計算から、収入・貯蓄率・不動産収入を考慮した高度なシミュレーションへ進化させる。

**詳細**:

#### FR-033-1: 収入ベースFIRE目標計算

- 従来の「年間支出 × 25」に加え、以下の計算方式をサポート:
  1. **標準FIRE**: 年間支出 × 25（4% ルール）
  2. **コーストFIRE**: 現資産が老後まで複利成長する前提で、追加貯蓄不要となる時点を計算
  3. **バリスタFIRE**: パートタイム収入で生活費の一部をカバーする前提で必要資産を計算
  4. **サイドFIRE**: 副業収入（事業・不動産）で生活費をカバーしつつ資産形成を継続
- ユーザーが目標とするFIREタイプを選択可能

**受け入れ条件**:

- 4種類のFIREタイプそれぞれで目標資産額を計算できる
- ユーザーがパラメータ（パートタイム収入額、副業収入額など）を指定できる
- 計算結果に達成までの年数と月数が含まれる

#### FR-033-2: シミュレーション条件設定

- ユーザーが以下のパラメータを設定可能:
  - 初期資産額（デフォルト: 最新スナップショット）
  - 月次貯蓄額（デフォルト: 過去12ヶ月の平均貯蓄額）
  - 年間運用利回り（デフォルト: 5%）
  - 年間インフレ率（デフォルト: 2%）
  - 追加収入（不動産、副業など）
  - 支出増加率（ライフイベント対応）
- パラメータセットを保存・再利用可能（シナリオ保存）

**受け入れ条件**:

- すべてのパラメータを個別に設定できる
- シナリオを JSON ファイルとして保存・読み込みできる
- デフォルト値が CSV データから自動計算される

#### FR-033-3: 複数シナリオの比較

- 最大5つのシナリオを同時にシミュレーション・比較
- 各シナリオで以下を計算:
  - FIRE達成年月
  - 必要な月次貯蓄額
  - 総資産の推移グラフ
...
#### FR-033-4: 受動的収入によるFIRE目標調整

- 不動産収入や配当収入などの受動的収入を考慮してFIRE目標を再計算
- 計算式: `必要資産 = (年間支出 - 年間受動的収入) × 25`
- 受動的収入が支出を上回る場合は「すでにFIRE達成」と判定

**受け入れ条件**:

- 受動的収入を考慮した目標資産額が正しく計算される
- 受動的収入が支出を上回る場合、達成済みと判定される
- 受動的収入の内訳（不動産、配当など）が表示される

#### FR-033-5: What-Ifシミュレーション

- ユーザーが仮定の変化を入力し、その影響をシミュレーション:
  - 「月次貯蓄額を +5万円 増やしたら?」
  - 「年間利回りが 3% だったら?」
  - 「支出が年 5% 増加したら?」
- 変化前後の達成年月と必要資産額を比較表示

**受け入れ条件**:

- What-If条件を複数組み合わせて設定できる
- 変化の影響が数値とグラフで表示される
- 達成年月の短縮効果が月数で表示される

---

### FR-034: 収入・キャッシュフロー分析のMCPツール

**目的**: 上記の収入分析とFIREシミュレーション機能をMCPツールとして公開し、AIエージェントが自然言語で利用できるようにする。

**詳細**:

#### FR-034-1: 収入分析MCPツール

- `get_income_summary(year, month)`: 月次収入サマリーを取得
- `get_annual_income_summary(year)`: 年次収入サマリーを取得
- `get_income_breakdown_by_category(start_date, end_date)`: 期間指定でカテゴリ別収入を取得

**受け入れ条件**:

- MCP経由で収入データを取得できる
- 返却データは JSON 形式で構造化されている

#### FR-034-2: 貯蓄率分析MCPツール

- `get_savings_rate(year, month)`: 月次貯蓄率を取得
- `get_savings_rate_trend(start_date, end_date)`: 期間の貯蓄率推移を取得
- `get_disposable_income(year, month)`: 可処分所得を取得

**受け入れ条件**:

- 貯蓄率が正しく計算される
- 推移データがグラフ化可能な形式で返却される

#### FR-034-3: 不動産キャッシュフローMCPツール

- `get_real_estate_cashflow(start_date, end_date, property_id=None)`: 不動産キャッシュフローを取得
- `calculate_real_estate_roi(property_id)`: 不動産ROIを計算

**受け入れ条件**:

- 不動産キャッシュフローが正しく計算される
- 複数物件の個別分析が可能

#### FR-034-4: 強化FIREシミュレーションMCPツール

- `simulate_fire_scenarios(scenarios)`: 複数シナリオを一括シミュレーション
- `calculate_fire_target_with_passive_income(passive_income)`: 受動的収入を考慮したFIRE目標を計算
- `what_if_fire_simulation(base_scenario, changes)`: What-Ifシミュレーションを実行

**受け入れ条件**:

- MCPツール経由でシミュレーションを実行できる
- 結果が構造化JSONで返却される

#### FR-034-5: 統合キャッシュフローレポート

- `generate_comprehensive_cashflow_report(year)`: 年次の総合キャッシュフローレポートを生成
  - 収入・支出・貯蓄の内訳
  - 貯蓄率の推移
  - 不動産キャッシュフロー
  - FIRE進捗状況
  - 達成見込み年月
- レポート形式: Markdown、JSON、HTML
...
### FR-037: 非同期画像ストリーミング安定化

**目的**: FR-016 (HTTP画像配信) を強化し、pytest(anyio)・FastAPI開発サーバ・MCPツール実行環境の全てでイベントループ競合による `RuntimeError`（"Runner is closed" / "Cannot run the event loop while another loop is running"）を排除し、安定した非同期画像ストリーミングを保証する。

**詳細**:

- 画像配信における非同期ストリーミング (`ImageStreamer.stream_bytes`) が単一イベントループ内で安全に動作し、二重ループ起動や閉鎖済みループ再利用を発生させない。
- 同期フォールバック (`stream_bytes_sync`) は必須ではない。保持する場合は単純委譲のみでイベントループ状態に影響しない。削除する場合はインポートエラーを起こさず後方互換 API（クラス初期化等）が維持される。
- FastAPI `StreamingResponse` 生成時のループ検出ロジックを最小化し、不要な try/except 分岐やループ再生成を行わない。
- 5件以上の並行リクエストで高負荷時も例外を発生させず全チャンクが送出される（FR-016 との整合）。
- テストコードは非同期テストと（必要な場合の）同期テストを分離し、anyio マークのスコープを適切化することでイベントループの teardown/再初期化競合を防ぐ。
- `ImageStreamer` を利用する他モジュール（キャッシュ、ツール、APIエンドポイント）からの利用パターンを網羅する統合テスト/単体テストを追加。

**受け入れ条件**:

- 全ストリーミング関連テスト実行時に `RuntimeError: Runner is closed` および "Cannot run the event loop while another loop is running" が発生しない。
- 5件並行アクセス試験で全レスポンスが 0.5 秒以内に完了し、例外なくチャンク列挙が終了する（ローカル環境、標準サイズ画像  < 5MB 換算）。
- `ImageStreamer` の分岐（async / optional sync）における行・分岐カバレッジが 90%以上。
- 同期フォールバック削除時（選択した場合）でも既存の `from household_mcp.streaming.image_streamer import ImageStreamer` インポートが成功する。
- StreamingResponse 経由で返却されるヘッダ（`Content-Type`, 任意 `Content-Disposition`）が正しく設定される。

**テスト項目 (新規)**:

- TS-050: 非同期ユニットテスト（`async for` チャンク取得）がイベントループ例外を起こさない。
- TS-051: FastAPI 統合テストで 5 並行ストリーム要求が全て成功。
- TS-052: 同期フォールバック削除（または保持）選択シナリオでインポートが壊れないことを確認。
- TS-053: カバレッジレポートで `image_streamer.py` の分岐/行カバレッジ 90%以上。
- TS-054: StreamingResponse 経由のヘッダ検証（`Content-Type` 等）が成功。
...
- **ステータス**: ✅ 承認済み
- **承認日**: 2025-11-19
- **概要**: 非同期画像ストリーミングを全実行コンテキストで安定化しイベントループ競合を排除。
- **受け入れ条件**: 全て明記済み（TS-050〜TS-054）
- **次ステップ**: `design.md` に設計追加（asyncループ戦略/テスト分離）、`tasks.md` に実装タスク展開後リファクタ実施
