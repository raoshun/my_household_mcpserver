...
- **資産管理機能** ✨ **NEW**: 複数資産クラス（現金、株式、投信、不動産、年金）の残高管理と配分分析
  - REST API で資産レコードの CRUD 操作
  - 月次資産サマリー取得（クラス別残高、合計）
  - 資産配分分析（割合・トレンド）
  - CSV エクスポート機能
  - Web UI で対話的な資産管理（4つのタブ、グラフ表示、リアルタイム検索）
  - モバイル・タブレット・PC 対応レスポンシブデザイン
- **高度な分析機能** ✨✨ **PHASE 18 NEW**:
  - **収入分析**: 給与・事業・不動産など収入源別の分析と貯蓄率計算
  - **強化FIRE計算**: 実支出データに基づく高精度なFIRE達成予測（4つのFIREタイプ対応）
  - **What-Ifシミュレーション**: 「もし月5万円節約したら？」などのシナリオ比較
  - **支出パターン分析**: 定期・変動・異常支出の自動分類、季節性検出、トレンド分析
...
| **✨ Phase 15 新ツール**     |                                                                                           |                 |
| `calculate_fire_index`      | **FIRE計算**: 現在資産と月貯蓄から経済的自由到達月を計算（複利・インフレ考慮、3シナリオ） | JSON            |
| `simulate_scenarios`        | **シナリオ分析**: 支出削減・収入増加など複数シナリオをROIで比較、最適施策を提案           | JSON            |
| `analyze_spending_patterns` | **パターン分析**: 支出を定期・変動・異常に分類、季節性・トレンド検出                      | JSON            |
...
- [API リファレンス](docs/api.md) - HTTPエンドポイント仕様
  - 家計簿API
  - **資産管理API** ✨ NEW
    - 8個のエンドポイント（CRUD、集計、分析、エクスポート）
    - リクエスト/レスポンス例
    - curl使用例
    - エラーレスポンス定義
    - パフォーマンス特性（NFR-022, 023, 024）
...
詳細は [HTTPサーバー実装](backend/src/household_mcp/web/http_server.py) を参照してください。
...
資産管理機能は、複数の資産クラス（現金、株式、投信、不動産、年金）の残高を記録・管理し、時系列で資産配分分析を行うための包括的な機能セットです。

### 構成

| 層                   | コンポーネント          | 説明                         |
| -------------------- | ----------------------- | ---------------------------- |
| **データベース**     | SQLite + SQLAlchemy ORM | 永続化層（論理削除パターン） |
| **ビジネスロジック** | `AssetManager` クラス   | CRUD、集計、分析メソッド     |
| **REST API**         | FastAPI 8エンドポイント | HTTP インターフェース        |
| **Web UI**           | HTML/CSS/JavaScript     | 対話的な資産管理アプリ       |

### 主なエンドポイント

| メソッド | パス                       | 説明                             |
| -------- | -------------------------- | -------------------------------- |
| POST     | `/api/assets/records`      | 資産レコード追加                 |
| GET      | `/api/assets/records`      | レコード一覧（フィルタ対応）     |
| PUT      | `/api/assets/records/{id}` | レコード更新                     |
| DELETE   | `/api/assets/records/{id}` | レコード削除（論理削除）         |
| GET      | `/api/assets/summary`      | 月次サマリー（クラス別残高）     |
| GET      | `/api/assets/allocation`   | 資産配分分析（パーセンテージ）   |
| GET      | `/api/assets/export`       | CSV エクスポート（フィルタ対応） |
| GET      | `/api/assets/classes`      | 資産クラス一覧                   |

### Web UI 機能

#### 4つのメインタブ

1. **概要**: 統計サマリー + Chart.js グラフ表示
   - 総資産額、前月比、最大資産額、クラス数
   - 資産配分円グラフ
   - クラス別残高棒グラフ

2. **レコード**: CRUD 管理テーブル + リアルタイム検索
   - 新規レコード登録フォーム
   - 検索・フィルタリング
   - 編集・削除機能（モーダル）

3. **配分**: 資産配分分析テーブル + ドーナツ図
   - クラス別残高・比率・プログレスバー
   - Chart.js ドーナツ図
...
- **PC（1024px+）**: 4列グリッド、全機能表示
- **タブレット（768px～1023px）**: 2列表示、調整されたグラフ
- **スマホ（480px～767px）**: 1列表示、折りたたみメニュー

### パフォーマンス特性

| NFR         | 目標                 | 実測値                 |
| ----------- | -------------------- | ---------------------- |
| **NFR-022** | API レスポンス < 1秒 | 50～500ms              |
| **NFR-023** | グラフ生成 < 3秒     | 100～300ms（Chart.js） |
| **NFR-024** | 1000件集計 < 1秒     | 800ms以下              |

### テストカバレッジ

- **ユニットテスト**: 24個（AssetManager 97%カバレッジ）
  - CRUD 操作テスト
  - 集計・分析ロジック
  - エクスポート機能
  - エッジケース処理

- **統合テスト**: 21個（全エンドポイント検証済み）
  - エンドポイント存在確認
  - パラメータ検証
  - エラーハンドリング
  - フィルタリング動作
...
- [使用ガイド](docs/usage.md#資産管理機能) - Web UI の詳細な操作手順
- [API リファレンス](docs/api.md#資産管理-api) - エンドポイント仕様、curl 例
